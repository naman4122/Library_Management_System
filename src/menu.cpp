...
void menu::display_spb(char n[])
{
	...
	{
		if(strcmpi(bk.returnbookno(),n)==0)
		{
			bk.show_book();
		 	flag=1;
		}
...
void menu::display_sps(char n[])
{
	...
	{
		if((strcmpi(st.returnadmissionno(),n)==0))
		{
			st.show_student();
			flag=1;
		}
...
void menu::book_issue()
{
	...
	fp1.open("book.txt",ios::in|ios::out);
    	while(fp.read((char*)&st,sizeof(student)) && found==0)
       	{
		if(strcmpi(st.returnadmissionno(),sn)==0)
		{
			found=1;
			if(st.returntoken()==0)
			{
				...
				{
					if(strcmpi(bk.returnbookno(),bn)==0)
					{
						bk.show_book();
						flag=1;
						st.addtoken();
						st.getstbno(bk.returnbookno());
       						int pos=-1*sizeof(st);
						fp.seekp(pos,ios::cur);
						fp.write((char*)&st,sizeof(student));
    ...
    while(fp.read((char*)&st,sizeof(student)) && found==0)
       {
	    if(strcmpi(st.returnadmissionno(),sn)==0)
	    {
		  ...
		  if(flag==0)
			...
			cout<<"!! No book is issued..please check Records !!";
		}
	   }
      if(found==0)
	cout<<"%%% Student record not exist... %%%";
